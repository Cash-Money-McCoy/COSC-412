...
var Game = {
	...
	arrowDelay: Phaser.Timer.SECOND * 0.33, // minimum time between player mouse clicks
...
Game.initWorld = function(data){ // Initialize the game world based on the server data
    ...
    Game.messageIn((Game.isNewPlayer ? 'Welcome to A-Quest!' : 'Welcome back!' ));
...
Game.enableArrow = function(){
    this.arrowEnabled = true;
};

Game.disableArrow = function() {
    this.arrowEnabled = false;
};

Game.handleArrow = function(){
    // If click is enabled, return true to the calling function to allow player to click,
    // then disable any clicking for time clickDelay
    if (this.arrowEnabled){
        // re-enable the click after time clickDelay has passed
        game.time.events.add(this.arrowDelay, this.enableArrow, this);
        Game.disableArrow();
        return true;
    }
    return false;
...
Game.update = function(){ // Main update loop of the client
	...
	if (Game.upKey.isDown){
		if (Game.handleArrow()){
			var end = Game.computeTileCoords(Game.player.x,Game.player.y - 32);
			Game.player.prepareMovement(end, 0, {action: 0}, 0, true); // true : send path to server
		}
		...
		if (Game.handleArrow()){
			var end = Game.computeTileCoords(Game.player.x - 32,Game.player.y);
			Game.player.prepareMovement(end, 0, {action: 0}, 0, true); // true : send path to server
		}
